...
export async function POST(request: Request) {
  try {
    ...
    const response = await fetch(`${BACKEND_URL}/api/telegram/stop`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      return NextResponse.json({ success: false, error: data.error || 'Failed to stop webhook' }, { status: response.status });
    }

    return NextResponse.json({ success: true, message: data.message || 'Webhook stopped' });
  } catch (error: any) {
    console.error('Telegram stop proxy error:', error);
